...
// Port Pin Constants, Motor Controller One
#define LEFT_MOTOR_A        35                                                 // GPIO35 pin 28 (J35) Motor 1 A
#define LEFT_MOTOR_B        36                                                 // GPIO36 pin 29 (J36) Motor 1 B
#define RIGHT_MOTOR_A       37                                                 // GPIO37 pin 30 (J37) Motor 2 A
#define RIGHT_MOTOR_B       38                                                 // GPIO38 pin 31 (J38) Motor 2 B
// Port Pin Constants, Motor Controler Two
...
const int turnTime=1600;                                                       //Time it takes for motor to make full rotation
...
void setup() {
   ...
   // Sets up motors - switch lefts and rights so my wheels are at the front
   //       Left  Right
   // Front M1    M2
   // Rear  M3    M4
...
void loop() {
   ...
   //Deactivates Motion Sensor for 2 microseconds
   digitalWrite(trigPin, LOW);
   delayMicroseconds(2);

  // Activiates Motion Sensor for 10 microseconds, then deactivates it
  digitalWrite(trigPin, HIGH);
  delayMicroseconds(10);
  digitalWrite(trigPin, LOW);

  // Reads the echoPin (Distance Sensor), returns the sound wave travel time in microseconds
  ...
  Serial.println(distance);
   ...
   if (millis() - lastTime > 10) {                // wait ~10 ms
      ...
      //If over 3 seconds have passed, robot will enter a switch loop, driving in different directions and breaking depending on the case.
      //The result is the robot driving in a square
      if(millis()>3000)
      {
      switch(driveIndex){

      case 0: //Bot counts current time and increases driveIndex
      collectTime=millis();
      driveIndex++;
      break;

      case 1: //Bot moves forward for 5 seconds
      botMove("forward",5000,&collectTime);
      break;

      case 2://Bot moves right for 5 seconds
      botMove("right",turnTime,&collectTime);
      break;

      case 3: //Bot moves forward for 5 seconds
      botMove("forward",5000,&collectTime);
      break;

      case 4: //Bot moves right for 5 seconds
      botMove("right",turnTime,&collectTime);
      break;

      case 5: //Bot moves forward for 5 seconds
      botMove("forward",5000,&collectTime);
      break;

      case 6: //Bot moves right for 5 seconds
      botMove("right",turnTime,&collectTime);
      break;

      case 7: //Bot moves forward for 5 seconds
      botMove("forward",5000,&collectTime);
      break;

      case 8: //Bot moves right for 5 seconds
      botMove("right",turnTime,&collectTime);
      break;
      }
      }
...
// 1200 is a reasonable target speed for now
   ...
   e = targetSpeed - wheelSpeeds[motorNumber];                              // position erro
...
//Calculates Speed motors should travel
void calculateSpeed() {
...
//Moves Bot forward with help of PID
void botForward(){
...
//Checks Conditions and Chooses which direction to move Bot
void botMove(char* direction, int seconds, int* relativeTime)
{
   ...
   //Used to check if code is functioning and if the timing of the PID is correct.
